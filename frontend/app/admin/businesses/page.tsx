            ...
            {filteredBusinesses.map((business) => (
              <div key={business.id} className="bg-white rounded-xl shadow-sm p-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {/* APPROVAL ACTIONS - For pending or draft businesses */}
                  {(business.status === 'pending' || business.status === 'draft') && (
                    <>
                      <button
                  ...
                  {/* OWNERSHIP VERIFICATION - Now handled in separate Verification Requests page */}
                  {business.status === 'active' && business.verification_status === 'pending' && (
                    <span className="px-3 py-2 text-sm text-orange bg-orange/10 rounded-lg font-medium">
